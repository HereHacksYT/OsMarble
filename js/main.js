...
let savedMaps = JSON.parse(localStorage.getItem('osmarble_v5')) || {
  "Çılgın OsMarble": {
    start: { x: 400, y: 80 },
    enableSideWalls: true,
    pieces: [
      { type: 'flat', x: 400, y: 160, angle: 0 },
      { type: 'ramp-down', x: 500, y: 280, angle: 0.35 },
      { type: 'spinner', x: 400, y: 420, angle: 0 },
      { type: 'lava', x: 400, y: 560, angle: 0 },
      { type: 'ramp-up', x: 300, y: 680, angle: -0.35 },
      { type: 'disappear', x: 400, y: 800, angle: 0 },
      { type: 'finish', x: 400, y: 950, angle: 0 }
    ]
  }
};
...
function saveMap() {
  ...
  localStorage.setItem('osmarble_v5', JSON.stringify(savedMaps));
  updateMapDropdown();
