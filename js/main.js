...
// Varsayılan Harita Verisi
let savedMaps = JSON.parse(localStorage.getItem('osmarble_maps_v2')) || {
  "OsMarble Özel Parkur": {
    start: { x: 400, y: 80 },
    enableSideWalls: true,
    pieces: [
      { id: 1, type: 'flat', x: 400, y: 160, angle: 0 },
      { id: 2, type: 'ramp-down', x: 500, y: 280, angle: 0.35 },
      { id: 3, type: 'spinner', x: 400, y: 420, angle: 0 },
      { id: 4, type: 'lava', x: 400, y: 560, angle: 0 },
      { id: 5, type: 'ramp-up', x: 300, y: 680, angle: -0.35 },
      { id: 6, type: 'disappear', x: 400, y: 800, angle: 0 },
      { id: 7, type: 'finish', x: 400, y: 950, angle: 0 }
    ]
  }
};
...
function resizeCanvas() {
  ...
  if (currentMode === 'menu' || currentMode === 'editor') {
...
function saveMap() {
  ...
  localStorage.setItem('osmarble_maps_v2', JSON.stringify(savedMaps));
  updateMapDropdown();
...
function loadSelectedMapToWorld() {
  ...
  const mapData = savedMaps[name] || { start: { x: canvas.width / 2, y: 100 }, pieces: [], enableSideWalls: true };
  startPoint = { ...mapData.start };
  enableSideWalls = mapData.enableSideWalls !== undefined ? mapData.enableSideWalls : true;
