window.addEventListener('DOMContentLoaded', () => {
  ...
  let editorMapPieces = [];
  let marbles = [];
  let startPoint = { x: window.innerWidth / 2, y: 100 };
  ...
  let savedMaps = JSON.parse(localStorage.getItem('osmarble_maps_v1')) || {
    "Örnek Parkur": {
      start: { x: window.innerWidth / 2, y: 80 },
      enableSideWalls: true,
      pieces: [
        { id: 1, type: 'flat', x: window.innerWidth / 2, y: 160, angle: 0 },
        { id: 2, type: 'ramp-down', x: window.innerWidth / 2 + 100, y: 280, angle: 0.35 },
        { id: 3, type: 'spinner', x: window.innerWidth / 2, y: 420, angle: 0 },
        { id: 4, type: 'lava', x: window.innerWidth / 2, y: 560, angle: 0 },
        { id: 5, type: 'ramp-up', x: window.innerWidth / 2 - 100, y: 680, angle: -0.35 },
        { id: 6, type: 'disappear', x: window.innerWidth / 2, y: 800, angle: 0 },
        { id: 7, type: 'finish', x: window.innerWidth / 2, y: 950, angle: 0 }
      ]
    }
  };
    ...
    canvas.addEventListener('pointerup', (e) => {
      ...
      if (!hasDragged && e.clientY > 80 && e.clientY < window.innerHeight - 80) {
        ...
        if (selectedTileType === 'eraser') {
          const found = allBodies.find(b => b.pieceType && Math.hypot(b.position.x - clickX, b.position.y - clickY) < 60);
          if (found) {
        ...
        if (selectedTileType === 'rotate_tool') {
          const found = allBodies.find(b => b.pieceType && Math.hypot(b.position.x - clickX, b.position.y - clickY) < 60);
          if (found) {
            const newAngle = found.angle + (Math.PI / 6);
  ...
  function saveMap() {
    ...
    localStorage.setItem('osmarble_maps_v1', JSON.stringify(savedMaps));
    updateMapDropdown();
  ...
  function loadSelectedMapToWorld() {
    ...
    const mapData = savedMaps[name] || { start: { x: window.innerWidth / 2, y: 100 }, pieces: [], enableSideWalls: true };
    startPoint = { ...mapData.start };
    enableSideWalls = mapData.enableSideWalls !== undefined ? mapData.enableSideWalls : true;
